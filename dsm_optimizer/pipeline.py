...
class DSMOptimizer:
    ...
    def sequence_stage(self, matrix, labels, stage1, algorithm, verbose=True):
        ...
        # ── Apply size constraints to the chosen candidate ────────────────────
        clusters = self._enforce_min_size(clusters, core_mat, c)
        clusters = self._enforce_max_size(clusters, core_mat, c)

        # Safety net: if still below min_k after enforcement, force spectral at min_k
        if len(set(clusters)) < min_k:
            _log(f"  Safety: forcing spectral at k={min_k}")
            clusters, _, fiedler = spectral_cluster(core_mat, k=min_k,
                                                     min_k=min_k, max_k=max_k)
            chosen_info['algorithm'] = 'spectral'
            chosen_info['forced_safety_net'] = True

        _log(f"    After size constraints: {len(set(clusters))} clusters")
